...
#define FTM_CUST_ACC "/data/bma222E"
/*----------------------------------------------------------------------------*/

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id bma222e_i2c_id[] = {{BMA222E_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_BMA222E={ I2C_BOARD_INFO("BMA222E", 0x18)};
...
static DEFINE_MUTEX(bma222e_mutex);
...
#else
	if((err = bma_i2c_read_block(client, BMA222E_REG_OFSX, ofs, BMA222E_AXES_NUM)))
	{
		GSE_ERR("error: %d\n", err);
	}
#endif
...
static int BMA222E_CheckDeviceID(struct i2c_client *client)
{
	...
	GSE_LOG("BMA222E_CheckDeviceID %d done!\n ", databuf[0]);

	#if 0
	if(databuf[0]!=BMA222E_FIXED_DEVID)
	{
		GSE_LOG("BMA222E_CheckDeviceID %d failt!\n ", databuf[0]);
		return BMA222E_ERR_IDENTIFICATION;
	}
	else
	{
		GSE_LOG("BMA222E_CheckDeviceID %d pass!\n ", databuf[0]);
	}
	#endif
	...
	if (res <= 0)
	{
		GSE_ERR("BMA222E_CheckDeviceID %d failt!\n ", BMA222E_ERR_I2C);
...
static int BMA222E_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if(bma_i2c_read_block(client, BMA222E_REG_DATA_FORMAT, databuf, 0x01))
	{
...
static int BMA222E_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	if(bma_i2c_read_block(client, BMA222E_REG_BW_RATE, databuf, 0x01))
	{
...
static int bma222e_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMA222E_SetPowerMode(client, enable_status);//false);//
		if(res != BMA222E_SUCCESS)
		{
			return res;
		}
	...
	GSE_LOG("bma222e_init_client OK!\n");
#ifdef CONFIG_BMA222E_LOWPASS
...
static int BMA222E_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		GSE_ERR("BMA222E_ReadSensorData bad operation sensor_power = %d!\n", sensor_power);
	...
	else
	{
		...
		if(fp != 0xfffffffe)//open file suscess
		{
			...
			filp_close(fp, NULL);	
			set_fs(old_fs);	
			sscanf(strbuf, "%02x %02x %02x", &data[BMA222E_AXIS_X], &data[BMA222E_AXIS_Y], &data[BMA222E_AXIS_Z]);
			obj->data[BMA222E_AXIS_X] -= data[BMA222E_AXIS_X];
			obj->data[BMA222E_AXIS_Y] -= data[BMA222E_AXIS_Y];
			obj->data[BMA222E_AXIS_Z] -= (data[BMA222E_AXIS_Z]-(obj->cvt.sign[BMA222E_AXIS_Z]*obj->reso->sensitivity));
		}
#ifdef BIRD_GSENSOR_BMA222E_X
		else{
			data[BMA222E_AXIS_X]=simple_strtol(BIRD_GSENSOR_BMA222E_X, NULL, 0);
			data[BMA222E_AXIS_Y]=simple_strtol(BIRD_GSENSOR_BMA222E_Y, NULL, 0);
			data[BMA222E_AXIS_Z]=simple_strtol(BIRD_GSENSOR_BMA222E_Z, NULL, 0);		
			obj->data[BMA222E_AXIS_X] -= data[BMA222E_AXIS_X];
			obj->data[BMA222E_AXIS_Y] -= data[BMA222E_AXIS_Y];
			obj->data[BMA222E_AXIS_Z] -= (data[BMA222E_AXIS_Z]-(obj->cvt.sign[BMA222E_AXIS_Z]*obj->reso->sensitivity));
		}
...
static int BMA222E_SET_CALIBRATION(struct i2c_client *client)
{
	struct bma222e_i2c_data *obj = (struct bma222e_i2c_data*)i2c_get_clientdata(client);
	char strbuf[BMA222E_BUFSIZE];
	s16   data[BMA222E_AXES_NUM+1];
	struct file *fp;
	BMA222E_ReadData(client, data);
	sprintf(strbuf, "%02x %02x %02x", data[BMA222E_AXIS_X], data[BMA222E_AXIS_Y], data[BMA222E_AXIS_Z]);	
	//write to nvram
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
	...
	fp->f_op->write(fp,
				  ...
				  &fp->f_pos);	
	filp_close(fp, NULL);	
	printk("BMA222E_SET_CALIBRATION value is %x %x %x\r\n",data[BMA222E_AXIS_X], data[BMA222E_AXIS_Y], data[BMA222E_AXIS_Z]);
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				else
				{
					...
					GSE_LOG("Gsensor not in suspend BMA222E_SetPowerMode!, enable_status = %d\n",enable_status);
					}else{
					enable_status = !sensor_power;
...

{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			BMA222E_SetPowerMode(client,true);	
			BMA222E_ReadSensorData(client, strbuf, BMA222E_BUFSIZE);
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				err = -EFAULT;
		...
		/*case GSENSOR_IOCTL_SET_CALIBRATION:
					...
					err = BMA222E_SET_CALIBRATION(client);
...
static void bma222e_early_suspend(struct early_suspend *h) 
{
	...
	if(bma_i2c_read_block(obj->client, BMA222E_REG_POWER_CTL, databuf, 0x01))
	{
		GSE_ERR("read power ctl register err!\n");
		mutex_unlock(&bma222e_mutex);
		return BMA222E_ERR_I2C;
	...
	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG("before BMA222E_SetPowerMode in suspend databuf = 0x%x\n",databuf[0]);
	if((err = BMA222E_SetPowerMode(obj->client, false)))
	{
		GSE_ERR("write power control fail!!\n");
		mutex_unlock(&bma222e_mutex);
	...
	if(bma_i2c_read_block(obj->client, BMA222E_REG_POWER_CTL, databuf, 0x01)) //for debug read power control register to see the value is OK
	{
		GSE_ERR("read power ctl register err!\n");
		mutex_unlock(&bma222e_mutex);
		return BMA222E_ERR_I2C;
	...
	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG("after BMA222E_SetPowerMode suspend err databuf = 0x%x\n",databuf[0]);
	sensor_suspend = 1;
	mutex_unlock(&bma222e_mutex);
	BMA222E_power(obj->hw, 0);
}
...
static void bma222e_late_resume(struct early_suspend *h)
{
	...
	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG("before bma222e_init_client databuf = 0x%x\n",databuf[0]);
	...
	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG("after bma222e_init_client databuf = 0x%x\n",databuf[0]);
	sensor_suspend = 0;
	mutex_unlock(&bma222e_mutex);
	atomic_set(&obj->suspend, 0);    
}
...
#ifdef BIRD_GSENSOR_BMA222E_DIR
	obj->hw->direction = simple_strtol(BIRD_GSENSOR_BMA222E_DIR, NULL, 0);	
#endif	
...
static int bma222e_i2c_remove(struct i2c_client *client)
{
	...
	{
		GSE_ERR("bma222e_delete_attr fail: %d\n", err);
	}
#else
...
static int  bma222e_local_init(void)
{
	...
	if(i2c_add_driver(&bma222e_i2c_driver))
	{
		printk("mycat add bma222e driver error\n");
		return -1;
	}
